...
int main(int argc, char *argv[]) {
	...
	err = gut_parse_double(argv[1], &val);

	// WARNING: The following pieces of code assume
	// sizeof(unsigned long long) == sizeof(double).

	if (err) {
		fprintf(stderr, "Error: conversion failed (%d)\n", err);
		if (err == GUT_E_RANGE) {
			fprintf(stderr, "(value returned: %.16g [%llx])\n", val,
			    *(unsigned long long *) &val);
		}
		exit(EXIT_FAILURE);
	}

	printf("gut_parse_double() returned %.16g [%llx]\n", val,
	    *(unsigned long long *) &val);
